        ...
        <table class="table table-bordered" id="order-dataTable" width="100%" cellspacing="0">
          ...
          <tbody>
            @foreach($reviews as $review)
                <tr>
                <td>{{$review->id}}</td>
                <td>{{ $review->user_info['name'] ?? 'N/A' }}</td> <!-- Use 'N/A' or any default value if user_info is null -->
                <td>{{ optional($review->product)->title }}</td> <!-- Use optional() to safely access properties of potentially null objects -->
                <td>{{$review->review}}</td>
                    <td>
                     <ul style="list-style:none">
                          @for($i=1; $i<=5;$i++)
                          @if($review->rate >=$i)
                            <li style="float:left;color:#145bb3;"><i class="fa fa-star"></i></li>
                          @else
                            <li style="float:left;color:#145bb3;"><i class="far fa-star"></i></li>
                          @endif
                        @endfor
                     </ul>
                    </td>
